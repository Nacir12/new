...
/*Observa que las variables *a3, *a4, y *a5 se declaran como ap untadores de 
...
Lectura(a5);         /*En este caso se pasa el apuntador de tipo estructura alumno a5
a la función Lectura. */

Lectura(&a6);   /*En este caso se pasa la variable de tipo estructura alumno a6, 
a la función Lectura. Observa que en este caso debemos utilizar el operador de dirección para preceder a la variable. */
printf("\nDatos del alumno 3\n");
...
gets((*a).carrera);
printf("Ingrese el promedio del aalumno: ");
scanf("f", &a->promedio);
fflush(stdin);
